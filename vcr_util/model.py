...
class AttentionQA(Model):
    def __init__(self,
        ...
        self.detector = SimpleDetector(pretrained=True, average_pool=True, semantic=class_embs, final_dim=2048)
        ###################################################################################################
    ...
    def forward(self,
        ...
        print("obj reps are: ", obj_reps['obj_reps'])
        x = metadata[0]['img_fn'] + '.npz'
        image_nm = "datas/"+ x.split('/')[len(x.split('/'))-1]
        print("image name is: ", image_nm)
        np.savez(image_nm, obj_reps)
        print("npz filed saved to the: ", image_nm)
